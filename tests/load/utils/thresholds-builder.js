export default class ThresholdsBuilder {
  ...
  addThreshold(testType, config) {
    ...
    this.thresholds[`checks{scenario:${testType}}`] = [`rate>${this.getCheckPassRate(testType)}`];
    this.thresholds[`http_req_failed{scenario:${testType}}`] = [
      `rate<${this.getErrorRateThreshold(testType)}`,
    ];
