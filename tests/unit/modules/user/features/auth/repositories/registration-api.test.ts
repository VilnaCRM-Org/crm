...
describe('RegistrationAPI', () => {
  it('returns the API response on success', async () => {
    const postMock = jest.fn().mockResolvedValue({ email: 'ada@example.com', fullName: 'Ada Lovelace' });
    const api = new RegistrationAPI(makeHttpsClient(postMock));

    const result = await api.register({ email: 'ada@example.com', fullName: 'Ada Lovelace', password: 'Password1' });
  ...
  it('wraps HttpError via handleApiError', async () => {
    const postMock = jest.fn().mockRejectedValue(new HttpError({ status: 409, message: 'Conflict' }));
    const api = new RegistrationAPI(makeHttpsClient(postMock));
