...
describe('login-slice reducer and thunk coverage', () => {
  ...
  it('handles full success flow (pending -> fulfilled)', async () => {
    ...
    const promise = store.dispatch(loginUser({ email: 'User@Test.com', password: 'pass123' }));
  ...
  it('clears auth state via logout action', () => {
    ...
    store.dispatch({
      ...
      payload: { token: 'tok', email: 'u@u.com' },
    });
